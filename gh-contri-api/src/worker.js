...
const renderLegend = (scheme, x, y) => {
  ...
  return `
    <g aria-hidden="true">
      <text x="${x - 36}" y="${y + 9}" font-size="9" fill="#4a5568">${labels[0]}</text>
      ${blocks}
      <text x="${x + scheme.length * (CELL_SIZE + 2) + 4}" y="${y + 9}" font-size="9" fill="#4a5568">${labels[1]}</text>
    </g>`;
};
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${label}">
  <title>${label}</title>
  <rect width="100%" height="100%" fill="${scheme[0]}" />
  <text x="${LEFT_PADDING}" y="18" font-size="12" font-family="'Segoe UI', Tahoma, sans-serif" fill="#111827">${username}'s GitHub contributions</text>
  <text x="${LEFT_PADDING}" y="30" font-size="10" font-family="'Segoe UI', Tahoma, sans-serif" fill="#4a5568">${totalContributions} contributions in the last year</text>
  ${cells}
